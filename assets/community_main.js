// Configuration
const API_URL = '/FOODWISE/index.php?route=community/api/reviews';
const RESPONSE_API_URL = '/FOODWISE/index.php?route=community/api/responses';
const RECIPES_API_URL = `${API_URL}&recipes=1`;
const currentUserId = (typeof FOODWISE_USER_ID !== 'undefined') ? FOODWISE_USER_ID : 2;
...
function displayResponsesForReview(reviewId, responses) {
    ...
    function renderResponse(response, level = 0) {
        ...
        let html = `
            <div class="response-card" id="response-${response.id}" style="margin-left: ${marginLeft}px; background: ${bgColor}; border-left: ${level > 0 ? '3px solid ' + borderColor : 'none'}; margin-bottom: 12px;">
    ...
    fetch(RECIPES_API_URL)
        ...
        .then(data => {
            ...
            for (let i = 0; i < data.data.length; i++) {
                ...
                const id = recipe.id_recette;
                const name = String(recipe.nom || '').trim();
                if (!id || !name) continue;
        ...
        .catch(() => {
            select.innerHTML = '<option value="">Erreur de chargement des recettes</option>';
        });
}
...
const REVIEW_LIKE_API_URL = '/FOODWISE/index.php?route=community/api/likes';
