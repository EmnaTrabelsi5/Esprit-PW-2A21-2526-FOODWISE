...
class RecetteController
{
    ...
    public function index(): void
    {
        ...
        require_once __DIR__ . '/../model/Favori.php';
        if (session_status() === PHP_SESSION_NONE) session_start();
        $favorisIds = !empty($_SESSION['user_id'])
            ? Favori::getIdsByUser((int)$_SESSION['user_id'])
            : [];
    ...
    public function show(int $id): void
    {
        ...
        require_once __DIR__ . '/../model/Favori.php';
        if (session_status() === PHP_SESSION_NONE) session_start();
        $estFavori = !empty($_SESSION['user_id'])
            ? Favori::isFavori((int)$_SESSION['user_id'], $id)
            : false;
    ...
    public function create(bool $admin = false): void
    {
        ...
        if (empty($_POST['ingredients'])) {
            ...
            if (empty($erreurs)) {
                ...
                try {
                    if (session_status() === PHP_SESSION_NONE) session_start();
                    $data['id_user'] = $_SESSION['user_id'] ?? null;
...
/* ════════════════════════════════════════════════
   FAVORIS — Toggle via AJAX (retourne JSON)
   Route : POST /recettes/{id}/favori
   ════════════════════════════════════════════════ */
public function toggleFavori(int $id): void
{
    ...
    require_once __DIR__ . '/../model/Favori.php';
    $action = Favori::toggle((int)$userId, $id);
...
/* ════════════════════════════════════════════════
   FAVORIS — Page liste des favoris de l'utilisateur
   Route : GET /favoris
   ════════════════════════════════════════════════ */
public function mesFavoris(): void
{
    ...
    $recettes   = $userId ? Favori::getRecettesByUser((int)$userId) : [];
    $favorisIds = $userId ? Favori::getIdsByUser((int)$userId) : [];

    $pageTitle  = 'Mes Favoris';
    $activeNav  = 'favoris';
    $backoffice = false;

    include 'view/recipebook/front/favoris.php';
}
