...
<aside class="sidebar">
  ...
  <nav class="sidebar-nav">
    <?php if (!$backoffice): ?>
      ...
      <a href="/FOODWISE/?route=recettes" 
   class="nav-item <?= $activeNav === 'recettes' ? 'active' : '' ?>">
      ...
      <a href="/FOODWISE/?route=module2/front/profil.edit" class="nav-item <?= $activeNav === 'objectifs' ? 'active' : '' ?>">
      ...
      <a href="/FOODWISE/?route=offres" class="nav-item <?= $activeNav === 'marche' ? 'active' : '' ?>">
      ...
      <a href="/FOODWISE/?route=community" class="nav-item <?= $activeNav === 'community' ? 'active' : '' ?>">
      ...
      <a href="/FOODWISE/?route=foodlog/journal" class="nav-item <?= $activeNav === 'foodlog_journal' ? 'active' : '' ?>">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>
        Journal alimentaire
      </a>
      <a href="/FOODWISE/?route=foodlog/suivi" class="nav-item <?= $activeNav === 'foodlog_suivi' ? 'active' : '' ?>">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg>
        Suivi Santé
      </a>
      <a href="/FOODWISE/?route=foodlog/resume" class="nav-item <?= $activeNav === 'foodlog_resume' ? 'active' : '' ?>">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M18 20V10M12 20V4M6 20v-6"/></svg>
        Résumé du jour
      </a>
      <a href="/FOODWISE/?route=module2/front/profil.edit" class="nav-item <?= $activeNav === 'nutrition' ? 'active' : '' ?>">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="4"/><path d="M12 3v2M12 19v2M3 12h2M19 12h2"/></svg>
        Mes Objectifs
      </a>
    <?php endif; ?>
...
<header class="topbar">
  ...
  <?php
    ...
    $profileRoute = '/FOODWISE/?route=module2/front/mon_profil';
  ?>
  ...
  <nav class="topbar-nav">
   ...
   Mes Recettes
    ...
    <a href="/FOODWISE/?route=module2/front/profil.edit" class="<?= $activeNav === 'objectifs' ? 'active' : '' ?>">Mes Objectifs</a>
    <a href="/FOODWISE/?route=offres" class="<?= $activeNav === 'marche' ? 'active' : '' ?>">Le Marché</a>
    <a href="/FOODWISE/?route=community" class="<?= $activeNav === 'community' ? 'active' : '' ?>">Communauté</a>
  </nav>
  <div class="topbar-user">
    <?php if ($currentUser): ?>
      <a href="<?= htmlspecialchars($profileRoute, ENT_QUOTES, 'UTF-8') ?>" style="display:inline-flex; align-items:center; gap:0.5rem; text-decoration:none; color:inherit;">
        <?php if ($userImage): ?>
          <img src="/FOODWISE/assets/uploads/<?= htmlspecialchars($userImage, ENT_QUOTES, 'UTF-8') ?>" alt="Avatar de <?= htmlspecialchars($userName, ENT_QUOTES, 'UTF-8') ?>" style="width:32px; height:32px; border-radius:50%; object-fit:cover;">
        <?php else: ?>
