...
<div class="recipe-grid">
  ...
  <a href="/FOODWISE/?route=recettes/<?= $r->id_recette ?>" class="recipe-card">
...
async function toggleFavori(event, btn, recetteId) {
    ...
    try {
        const resp = await fetch('/FOODWISE/?route=recettes/' + recetteId + '/favori', {
            method: 'POST',
        ...
        if (data.error === 'non_connecte') {
            window.location.href = '/FOODWISE/?route=module2/front/connexion';
            return;
        }
